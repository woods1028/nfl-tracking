...
players <- base_dir %>% 
  ...
  pmap(function(x,y) mutate(x,season = y) %>% 
         setNames(c("nflId", "height", "weight", "birthDate", "collegeName", "position", 
                    "displayName", "season"))) %>% 
  bind_rows

players %>% 
  write_dataset(
    path = "reformatted/players_all",
    basename_template = paste0("part-{i}.", as.character("parquet")),
    format = "parquet",
    max_rows_per_file = 9e5
  )
